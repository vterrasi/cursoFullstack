...
public class analizarStringProComentado {
    public static void main(String[] args) {

        String texto = "hola mundo moderno";
        String resultadoAnalisis = analizarStringProComentado(texto);
     ...
     */
    public static String analizarStringProComentado(final String entrada) {
        return Optional.ofNullable(entrada).filter(s -> !s.isBlank()).map(s -> s.chars() // IntStream para mejor performance en primitiva char
                        .filter(c -> !Character.isWhitespace(c))
                        .mapToObj(c -> (char) c)
                        .collect(Collectors.groupingBy(Function.identity(),Collectors.counting()
                        // Retorna Long, más seguro para strings masivos
                        ))
                )
                .map(analizarStringProComentado::formatearResultado)
                .orElse("Texto vacío");
    }
